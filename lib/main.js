...
let text = getUrlParameter('text');
if (!text) {
    text = 'Debe indicar una frase para comparar.';
}

$('#text').val(text.split('|')[0]);
...
$('#stop-button').click(function () {
    closeSocket();
    toggleStartStop();
    $("#result").html('<i class="fa fa-spinner"></i> Validando...');
    setTimeout(_ => showResult(), 3000);
});
...
function showResult() {
    ...
    text = text.split("|").map(v => v.toLowerCase().replaceAll('.', '').replaceAll(' ', '').replaceAll("\r", '').replaceAll("\n", ''));
    transcript = transcript.toLowerCase().replaceAll('.', '').replaceAll(' ', '').replaceAll("\r", '').replaceAll("\n", '');

    console.log(text)
    console.log(transcript)
    console.log(text.includes(transcript))

    if (text.includes(transcript)) {
        $("#result").html('<i class="fa fa-check"></i> Bien dicho!');
    }
    else {
        $("#result").html('<i class="fa fa-times"></i> Fallaste');
    }
}
